...
df1 <- select(ZA7500_v5_0_0_dta, v72, v80, age, v225, c_abrv, v243_ISCED_1)


country_names <- c(AL = "Albania", AM = "Armenia", AT = "Austria", AZ = "Azerbaijan",
                   BA = "Bosnia and Herzegovina", BG = "Bulgaria", BY = "Belarus",
                   CH = "Switzerland", CZ = "Czechia", DE = "Germany", DK = "Denmark",
                   EE = "Estonia", ES = "Spain", FI = "Finland", FR = "France",
                   GB = "Great Britain", GE = "Georgia", HR = "Croatia", HU = "Hungary",
                   IS = "Iceland", IT = "Italy", LT = "Lithuania", LV = "Latvia",
                   ME = "Montenegro", MK = "North Macedonia", NL = "Netherlands",
                   NO = "Norway", PL = "Poland", PT = "Portugal", RO = "Romania",
                   RS = "Serbia", RU = "Russia", SE = "Sweden", UA = "Ukraine")
...
age_descriptions <- c(`-10` = "multiple answers Mail",
                      `-9` = "no follow-up",
...
df1c <- df1c %>%
  mutate(age = ifelse(as.character(age) %in% names(age_descriptions), age_descriptions[as.character(age)], as.character(age)))
...
df1c <- df1c %>%
  mutate(Education = ifelse(as.character(v243_ISCED_1) %in% names(v243_ISCED_1_descriptions),
...
df1c <- df1c %>%
  mutate(respondent_sex = ifelse(as.character(v225) %in% names(respondent_sex_descriptions),
...
df1c <- df1c %>%
  mutate(jobs_are_scarce = ifelse(as.character(v80) %in% names(jobs_are_scarce_descriptions),
...
df1c <- df1c %>%
  mutate(child_suffers = ifelse(as.character(v72) %in% names(child_suffers_descriptions),
...
df1c <- df1c %>%
  mutate(respondent_sex = case_when(
    respondent_sex %in% c("multiple answers Mail", "no follow-up", "follow-up non response", 
                          "other missing", "item not included", "not applicable", 
                          "no answer", "dont know") ~ "unknown",
...
kable(sex_table, format = "simple", col.names = c("Sex", "Frequency", "Percentage (%)"))

# Assuming df1c$respondent_sex has been properly modified to contain "male", "female", and "unknown" as categories
sex_freq <- table(df1c$respondent_sex) # Frequency table for respondent_sex
...
kable(country_frequency, format = "simple", col.names = c("Country", "Frequency"))
...
education_df$Percentage <- (education_df$Frequency / sum(education_df$Frequency)) * 100
...
knitr::kable(education_df, format = "simple", col.names = c("Education", "Frequency", "Percentage (%)"))
...
# Add a percentage column
child_suffers_df$Percentage <- (child_suffers_df$Frequency / sum(child_suffers_df$Frequency)) * 100

# Round the percentages for better readability
child_suffers_df$Percentage <- round(child_suffers_df$Percentage, 2)


knitr::kable(child_suffers_df, format = "simple", 
...
# Add a percentage column
jobs_are_scarce_df$Percentage <- (jobs_are_scarce_df$Frequency / sum(jobs_are_scarce_df$Frequency)) * 100

# Round the percentages for better readability
jobs_are_scarce_df$Percentage <- round(jobs_are_scarce_df$Percentage, 2)


knitr::kable(jobs_are_scarce_df, format = "simple", 
...
kable(categorical_summary, format = "html", 
...
# If the "age" variable is already numeric but uses a special value (e.g., 83, 84, etc.) to indicate "82 and older"
df1c <- df1c %>%
  mutate(age = ifelse(age >= 82, 82, age))

# This will replace any age values of 82 and older with just 82, keeping the age column numeric.
